...
#     labels = (hed_tile[:, :, 0] > 0.001).astype(np.uint8)
#     # result[:, :, 0] = labels  # Set only the first channel with labels
#     return labels * 255 * mask

# def get_mask(channel):
#     dask_arr = da.from_array(channel, chunks=(256, 256, 3))
...
#     labels, _ = model.predict_instances(normalize(channel),n_tiles=(28,23,1))
#     return labels

@dask.delayed
def process_tile(tile):
    # mask = get_mask(tile)
    labels = (tile[:, :, 0] > 0.001).astype(np.uint8)
    labels = labels * 255
    return labels #  * mask

# def get_mask(tile):
#     model = StarDist2D.from_pretrained('2D_versatile_he')
#     labels, _ = model.predict_instances(normalize(tile))
#     retu

def process_array(arr):
    hed_arr = da.from_delayed(rgb2hed_custom(arr), shape=arr.shape, dtype=np.float32)
    print("rgb2hed_custom done")
    # mask_arr = da.map_blocks(get_mask, arr, dtype=np.uint8)
    result_arr = da.from_delayed(process_tile(hed_arr), shape=arr.shape[:2], dtype=np.uint8)
    return result_arr
...
def get_labels(pyramid):
    return [da.from_delayed(dask.delayed(np.zeros)(arr.shape[:2]), shape=arr.shape[:2], dtype=np.float32) for arr in pyramid]
...
def reader_function(path):
    ...
    for _path in paths:
        if _path.endswith('.svs'):
            ...
            labels = get_labels(pyramid)
            labels[0] = process_array(pyramid[0])
            print(labels)
            add_kwargs = {}
